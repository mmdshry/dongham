...
const TD = 'padding:8px 10px;border-bottom:1px solid rgba(94,114,98,0.12);text-align:right;vertical-align:top';
const TH = `${TD};background:rgba(94,114,98,0.08);font-weight:700;color:#5E7262`;
...
export function buildPeriodReportHtml(
  ...
  const balanceRows =
    model.balances.length === 0
      ...
      : model.balances
          .map((row) => {
            const color = row.amount < -0.5 ? '#BE123C' : row.amount > 0.5 ? '#5E7262' : '#2A3E34';
            return `<tr>
  ...
  const settlementItems =
    model.settlements.length === 0
      ...
      : `<ul style="margin:10px 0 0;padding:0;list-style:none">${model.settlements
          .map(
            (s) =>
              `<li style="margin:0 0 8px;padding:10px 12px;border-radius:12px;background:rgba(94,114,98,0.08);font-size:14px;line-height:1.7">${escapeHtml(s.sentence)}</li>`,
          )
          .join('')}</ul>`;
  ...
  const tables =
    ...
    <section style="margin-top:22px">
      <h2 style="margin:0 0 8px;font-size:16px;color:#5E7262">هزینه‌ها</h2>
      <table style="${TABLE}">
        <thead>
          <tr>
            ${th('تاریخ')}
    ...
    <section style="margin-top:22px">
      <h2 style="margin:0 0 8px;font-size:16px;color:#5E7262">پرداخت‌ها</h2>
      <table style="${TABLE}">
        <thead>
          <tr>
            ${th('تاریخ')}
  ...
  return `<article dir="rtl" lang="fa" data-period-report="${variant}" style="width:${REPORT_WIDTH_PX}px;box-sizing:border-box;padding:28px 32px 36px;background:${REPORT_BG};color:#2A3E34;font-family:Vazirmatn,Tahoma,sans-serif;line-height:1.6">
    <header>
      <div style="display:flex;justify-content:space-between;align-items:baseline;gap:12px;font-size:13px;color:#5E7262">
        <span style="font-weight:800">دونگ‌هام</span>
        <span>${escapeHtml(model.exportDate)}</span>
      </div>
      <h1 style="margin:10px 0 6px;font-size:28px;line-height:1.3;color:#5E7262">${escapeHtml(model.title)}</h1>
      <p style="margin:0;font-size:14px">${toPersianDigits(model.memberCount)} عضو · ${escapeHtml(model.currencyFa)}</p>
    </header>
    <section style="margin-top:20px;padding:14px 16px;border-radius:16px;background:#FFFAF5;border:1px solid rgba(94,114,98,0.12)">
      <p style="margin:0;font-size:13px;color:#64748b">جمع هزینه‌ها</p>
      <p style="margin:4px 0 0;font-size:22px;font-weight:800;color:#5E7262">${escapeHtml(model.totalLabel)}</p>
    </section>
    <section style="margin-top:22px">
      <h2 style="margin:0 0 8px;font-size:16px;color:#5E7262">حساب اعضا</h2>
      <table style="${TABLE}">
        <thead>
          <tr>
            ${th('نام')}
    ...
    <section style="margin-top:22px">
      <h2 style="margin:0 0 4px;font-size:16px;color:#5E7262">تسویه حساب</h2>
      ${settlementItems}
    </section>
    ${tables}
  </article>`;
